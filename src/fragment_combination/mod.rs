...
pub fn generate_combinations<T: Groupable + Clone + Send + Sync + 'static>(fragments: &[T], k: usize) -> Box<dyn Iterator<Item = Vec<T>> + Send> {
    ...
    Box::new(
        (0..n).flat_map(move |i| {
            ...
            if k == 1 {
                ...
                Box::new(generate_combinations_recursive(&rest, k - 1, new_used_groups).map(move |c| {
