...
fn test_group_aware_combinations() {
    let f1 = Fragment { text: "a".to_string(), group_id: Some("G1".to_string()) };
    let f2 = Fragment { text: "b".to_string(), group_id: Some("G1".to_string()) };
    let f3 = Fragment { text: "c".to_string(), group_id: Some("G2".to_string()) };
    ...
    for combo in combinations {
        ...
        for f in combo {
            if let Some(gid) = f.group_id {
                assert!(groups.insert(gid), "Each group should appear at most once in a combination");
            }
        }
    }
}
