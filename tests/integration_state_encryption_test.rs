...
fn test_sled_db_encryption_flow() {
    ...
    db.save_checkpoint("step_100")
        .expect("Failed to save checkpoint");
    db.mark_as_tried(&["pass1", "pass2"])
        .expect("Failed to mark as tried");
    ...
    assert!(db
        .is_tried(&["pass1", "pass2"])
        .expect("Failed to check if tried"));
}
...
fn test_tried_combinations_privacy() {
    ...
    for item in tried_tree.iter() {
        ...
        assert!(
            !k_str.contains("sensitive_pass"),
            "Plaintext combination found in database!"
        );
    }
}
