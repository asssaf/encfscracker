...
fn test_parallel_combination_skips_tried() {
    ...
    assert_eq!(
        ...
        5,
        "Should have skipped one combination"
    );
}
...
fn test_parallel_combination_resume_from_checkpoint() {
    ...
    let validator = move |c: &[String]| {
        ...
        if c == ["a".to_string(), "b".to_string()]
            || c == ["a".to_string(), "c".to_string()]
            || c == ["b".to_string(), "a".to_string()]
        {
            panic!(
                "Validator should NOT be called for checkpointed combinations {:?}",
                c
            );
        }
        false
    ...
    assert_eq!(
        call_count.load(Ordering::SeqCst),
        3,
        "Should have resumed from checkpoint 3"
    );
}
