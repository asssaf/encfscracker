...
fn test_validate_decrypted_key_valid() {
    ...
    assert!(validate_decrypted_key(&decrypted_data_bytes, &master_key_bytes, expected_checksum));
}
...
fn test_validate_decrypted_key_invalid_checksum() {
    ...
    assert!(!validate_decrypted_key(&decrypted_data_bytes, &master_key_bytes, incorrect_checksum));
}
